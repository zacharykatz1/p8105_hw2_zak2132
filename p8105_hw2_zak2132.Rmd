...
```{r}
# Note that `read_csv` imported a df with 789 columns and 0 rows, so had to use `read.csv` function instead
...
```{r}
# Import `unemployment` dataset, noting that similar to the prior dataset, we had to use `read.csv` instead of `read_csv` to get the correct/expected number of observations and variables
