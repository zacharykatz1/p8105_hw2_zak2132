...
ranging from 1 to 344. `Year`, `month`, and `date` are time variables.
...
precipitation in a given month, and ranges from 0.42 to 10.47. Mean
monthly precipitation is equal to 4.83 inches.
...
frame also has 6 observations on 3 variables, which are identical
...
to the S&P stock market index ranging from the years 200 to 2015.
...
dataset has 156 rows missing values for the `snp_close` variable and 12
